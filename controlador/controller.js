...
controller.actualizarusu=async(req,res,next)=>{
    const usid=req.body.uu;
    const log=req.body.ll;
    const pas=req.body.pp;
    const rol=req.body.rr;
    const password=await bcryptjs.hash(pas,8)
  
    cnn.query('UPDATE tbusuario set UsuLogin="'+log+'",UsuPassword="'+password+'",UsuRolTipo="'+rol+'" WHERE UsuId="'+usid+'"', async(err,respbb)=>{
  ...
  controller.eliminarusu=(req,res,next)=>{
      const usid=req.body.uu;
    
      cnn.query('DELETE  from tbusuario WHERE UsuId="'+usid+'"', async(err,respbb)=>{
